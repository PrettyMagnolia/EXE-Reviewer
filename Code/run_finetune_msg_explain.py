...
from causal_utils import load_model_and_tokenizer
...
wandb_project_name = "EXE-Reviewer"
...
def main(args):
    ...
    # create seq and tok causal model
    if args.has_explain:
        seq_model, seq_tokenizer = load_model_and_tokenizer('seq', args.causal_seq_model_path)
        tok_model, tok_tokenizer = load_model_and_tokenizer('tok', args.causal_tok_model_path)
    ...
    for epoch in range(1, args.train_epochs + 1):
        ...
        for _, _, train_dataloader in get_loaders(train_files, args, tokenizer, pool):  # WARNING: this is an iterator, to save memory
            for step, examples in tqdm(enumerate(train_dataloader, 1)):
                ...
                if args.has_explain:
                    params.update({
                        'explain_label': torch.tensor([ex.has_explain for ex in examples], dtype=torch.long).to(
                            local_rank),
                        'tokenizer': tokenizer,
                        'seq_model': seq_model,
...
if __name__ == "__main__":
    ...
    wandb.init(config=args, project=wandb_project_name, name=exp_name)
    main(args)
    wandb.finish()
