    ...
    def __init__(self, tokenizer, pool, args, file_path, samplenum=-1):
        ...
        if os.path.exists(savep):
            ...
            for i in range(len(data)):
                data[i]["idx"] = i
            logger.info("Tokenize examples: {}".format(file_path))
...
def filter_symbol(tokens_dict):
    filter_tokens = {}
    symbol_list = read_symbols(os.path.join(os.path.dirname(__file__), "evaluator/symbols.txt"))
    for token, value in tokens_dict.items():
        is_symbol = any(stopword in token for stopword in symbol_list)
        if not is_symbol:
            filter_tokens[token] = value
    return filter_tokens


def top_k_token_dict(tokens, values, k, rm_symbol=True):
    token_dict = {}
    for i in range(len(tokens)):
        token_dict[tokens[i]] = values[i]

    if rm_symbol:
        token_dict = filter_symbol(token_dict)

    return dict(list(token_dict.items())[:k])
